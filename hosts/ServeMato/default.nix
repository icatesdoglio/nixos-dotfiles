{
  ...
  mkQbitConfig = {
    ...
    pkgs.writeText "qBittorrent.conf" ''
      ...
      WebUI\Password_PBKDF2="__QBIT_PASSWORD_PLACEHOLDER__"
  ...
  in {
    ...
    after = ["local-fs.target" "network-online.target" "nss-lookup.target" "sops-nix.service"] ++ vpnDeps;
    ...
    serviceConfig = {
      ...
      Group = "media";
      UMask = "0002";
      ExecStartPre = pkgs.writeShellScript "qbit-start-pre-${user}" ''
        ${pkgs.coreutils}/bin/install -Dm600 ${configFile} ${profileDir}/qBittorrent/config/qBittorrent.conf
        ${pkgs.gnused}/bin/sed -i "s|__QBIT_PASSWORD_PLACEHOLDER__|$(cat ${passwordSecretPath})|" ${profileDir}/qBittorrent/config/qBittorrent.conf
      '';
      ExecStart = ''
...
in {
  ...
  */
  ...
  sops.secrets = {
    ...
    "qbt/password-encrypted" = {
      sopsFile = ../../secrets/qbt.yaml;
      format = "yaml";
      key = "qbtPassword-encrypted";
      path = "/run/secrets/qbt-password-encrypted";
      owner = "root";
      group = "media";
      mode = "0440";
    };
  ...
  */
  ...
  systemd.services.qbittorrent-public = mkQbitService {
    ...
    passwordSecretPath = config.sops.secrets."qbt/password-encrypted".path;
